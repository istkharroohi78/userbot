...
from ISTKHAR.helper.utility import get_arg
...
from ISTKHAR.database.rraid import *
...
async def pornspam(xspam: Client, e: Message): 
    ...
    count = int(counts)
    for _ in range(count):
         await xspam.send_message(e.chat.id, ISTKHAR)
...
async def raid(xspam: Client, e: Message):  
      ISTKHAR = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(ISTKHAR) == 2:
          counts = int(ISTKHAR[0])
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          ok = await xspam.get_users(ISTKHAR[1])
          id = ok.id
#          try:
...
#              return #remove # to enable this
          if int(id) in VERIFIED_USERS:
                ...
                text = f"Abe Lawde that guy part of istkhar."
                await e.reply_text(text)
          else:
              fname = ok.first_name
