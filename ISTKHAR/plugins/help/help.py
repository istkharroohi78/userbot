...
from ISTKHAR import app, CMD_HELP
from ISTKHAR.helper.PyroHelpers import ReplyCheck
from ISTKHAR.helper.utility import split_list
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "ISTKHAR-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
